...
(async () => {
  try {
    ...
    // Type assertion to handle optional cometChatCustomConfig
    const pkgData = pkg?.default as any;
    if (pkgData?.cometChatCustomConfig) {
      chatBuilderName = pkgData.cometChatCustomConfig.name ?? chatBuilderName;
      chatBuilderVersion = pkgData.cometChatCustomConfig.version ?? chatBuilderVersion;
    }
  } catch (e) {
    // fallback silently
  }
